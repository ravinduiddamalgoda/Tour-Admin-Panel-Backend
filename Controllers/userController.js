...
router.get("/getGuideProfile", authGuard, (req, res) => {
  ...
  router.put('/updateGuideProfile', authGuard, (req, res) => {
    const user = req.user;
    const { VehicleID, Languages, GuiType, Qualifications } = req.body;
    try {
      if (!user) {
        return res.status(400).send({ err: "User not logged in" });
      }
      ...
      connection.query(
        "UPDATE Guide SET VehicleID = ?, Languages = ?, GuiType = ?, Qualifications = ? WHERE UserID = ?",
        [VehicleID, Languages, GuiType, Qualifications, user.id],
        (err, result) => {
          if (err) {
